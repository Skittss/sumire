...
namespace sumire {
	...
	void SumiModel::loadGLTF(SumiDevice &device, const std::string &filepath, SumiModel::Data &data, bool isBinaryFile) {
		...
		for (uint32_t i = 0; i < data.nodes.size(); i++) {
			std::shared_ptr<Node> container = std::make_shared<Node>();
			container->idx = getLowestUnreservedGLTFNodeIdx(data); // -1 used as a flag for render-view only nodes.
			container->name = "-Y container (Generated by Sumire)";
			container->parent = nullptr;
			container->matrix[1][1] *= -1.0;
			container->children.push_back(data.nodes[i].get());
			container->children[0]->parent = container.get();
			data.nodes[i] = container;
			data.flatNodes.push_back(container);
		}
	...
	uint32_t SumiModel::getLowestUnreservedGLTFNodeIdx(SumiModel::Data &data) {
