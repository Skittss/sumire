...
namespace sumire::loaders {
    ...
    std::unique_ptr<SumiModel> GLTFloader::createModelFromFile(SumiDevice &device, const std::string &filepath) {
    ...
    void GLTFloader::loadModel(SumiDevice &device, const std::string &filepath, SumiModel::Data &data) {
		std::filesystem::path fp = filepath;
    ...
    void GLTFloader::loadGLTF(SumiDevice &device, const std::string &filepath, SumiModel::Data &data, bool isBinaryFile) {
		...
		for (uint32_t i = 0; i < data.nodes.size(); i++) {
			std::shared_ptr<SumiModel::Node> container = std::make_shared<SumiModel::Node>();
