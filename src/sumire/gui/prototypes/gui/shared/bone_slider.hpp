...
namespace kbf {
    ...
    inline bool ImBoneSliderH(const char* label, float width, float* value, float speed, float maxExtent, const char* fmt = "%.2f") {
        ...
        if (ImGui::IsKeyDown(ImGuiKey_LeftShift)) speed /= 10.0f; // Revert internal imgui fast tweak behaviour

        // Draw the slider
        ImGui::PushItemWidth(width);
        bool changed = ImGui::DragFloat(label, value, speed, -maxExtent, +maxExtent, fmt);
        ImGui::PopItemWidth();
